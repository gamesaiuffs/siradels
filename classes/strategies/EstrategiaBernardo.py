...
class EstrategiaBernardo(Estrategia):
    ...
    def escolher_personagem(estado: Estado) -> int:
        ...
        qntDistNobre = len(list(filter(lambda x: x.tipo_de_distrito == Td.Nobre, distritosContruidos)))

        if ouros.index(jogador.ouro) < len(ouros) // 2 and (ladrao := estado.tabuleiro.personagens[TipoPersonagem.Ladrao.value]) in personagens:
        ...
        if (len(personagens) < 3 or qntDistNobre >= 2) and (rei := TipoPersonagem.Rei.value) in personagens:
        ...
        #if (assasina := estado.tabuleiro.personagens[TipoPersonagem.Assassina.value]) in personagens:
        #    return estado.tabuleiro.baralho_personagens.index(assasina)
    ...
    def escolher_acao(estado: Estado, acoes_disponiveis: list[Ta]) -> int:
        ...
        while acoes_disponiveis[acao] in naoFazer or (acao == Ta.Laboratorio and min(estado.jogador_atual.cartas_distrito_mao, 0) > 2):
            acao = random.randint(0, len(acoes_disponiveis) - 1)
    ...
    def coletar_cartas(estado: Estado, cartas_compradas: list[CartaDistrito], qtd_cartas: int) -> int:
        return cartas_compradas.index(max(cartas_compradas, key=lambda x: x.valor_do_distrito))
    ...
    def construir_distrito(estado: Estado,
        ...
        sortedDistritos = sorted(qntDistritos.items(), key=lambda x: -x[1])
        
        #print(sortedDistritos)
        #try:
        #    for tipo, qnt in sortedDistritos:
        #        for i, carta in enumerate(todosDistritos):
        #            if tipo == carta.tipo_de_distrito:
        #                return i
    ...
    def construir_distrito_cardeal(estado: Estado, diferenca: int, i: int) -> int:
        return estado.jogador_atual.cartas_distrito_mao.index(min(estado.jogador_atual.cartas_distrito_mao, key=lambda x: x.valor_do_distrito))
    ...
    def habilidade_mago_jogador(estado: Estado, opcoes_jogadores: list[Jogador]) -> int:
        return opcoes_jogadores.index(max(opcoes_jogadores, key=lambda x: len(x.cartas_distrito_mao)))
    ...
    def habilidade_mago_carta(estado: Estado, opcoes_cartas: list[CartaDistrito]) -> int:
        return opcoes_cartas.index(min(opcoes_cartas, key=lambda x: -x.valor_do_distrito))
    ...
    def arsenal(estado: Estado, distritos_para_destruir: list[(CartaDistrito, Jogador)]) -> int:
        return distritos_para_destruir.index(max(distritos_para_destruir, key=lambda x: x[0].valor_do_distrito))
