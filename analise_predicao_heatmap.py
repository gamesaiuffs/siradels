...
if __name__ == "__main__":
    ...
    VARIABLE_NAME = "ouro_e_carta"  
    VARIABLE_RANGE = range(0, 11)  # Range de valores para a variável escolhida
    ...
    for INIT_NUM in range(1, 11):
        print(f"Init: {INIT_NUM}")
    
        EXP_NAME = f"Action choices heatmap in {NUM_EPISODES} episodes - variable: {VARIABLE_NAME}_exp={EXP_NUM}_init={INIT_NUM}_model={MODEL}" 
        NUM_ACTIONS = 8
        ...
        for i, var_value in enumerate(VARIABLE_RANGE):
            for _ in range(NUM_EPISODES // len(VARIABLE_RANGE)):  # Distribuindo episódios
                ...
                external_input = np.array([
                    ...
                    var_value,   # cartas_dist_mao (0 a 10)
